...
#             print("that's fine")
    ...
    def win_obs_speed(self, window_size):
        if len(self.obs_d) < window_size:
            return 0.0
    ...
    def win_est_fluctuation(self, window_size):
        if len(self.est_v) < window_size:
            return 0.0
        direction = (self.est_v[-window_size] >= 0)
        return np.std(self.est_v[-window_size:]) * (1 if direction else -1)
